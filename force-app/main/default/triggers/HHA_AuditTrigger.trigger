...
* @What  : HHA_AuditTrigger
* @Why   : To Handles all the customization involved on Health_Hazard_Assessment__c object.
* @When  : 07-FEB-2019
* @Where : From Health_Hazard_Assessment__c object Events
--------------------------------------------------------------------------------------------------------------------------------------
*  Modification Log:  
*  --------------------------------------------------------------------------------------------------------------------------------------
...
*  --------------------------------------------------------------------------------------------------------------------------------------
**/
trigger HHA_AuditTrigger on Health_Hazard_Assessment__c (before update,before delete,after insert,after update){
    ...
    if(!ADD_App_Settings__c.getInstance(UserInfo.getUserId()).Restrict_Triggers__c && ADDS4RiskManagementTriggerHandler.stopRecursionForClonedRecord == false){
        ...
        /* Before Delete */
            ...
            HHA_AuditTriggerHandler.beforeDelete(Trigger.old);
            CMPL123.AuditHandler.handleAudit();
         }
        /* After Delete */
